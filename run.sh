...
gnuplot -e "set terminal pngcairo size 1000,800; \
	 ...
	 set xlabel 'Input Size(array size(n))'; \
	    ...
	    set title 'heapsort vs quicksort vs mergesort with NORMAL DISTRIBUTION RANDOM inputs'; \
...
gnuplot -e "set terminal pngcairo size 1000,800; \
	 ...
	 set xlabel 'Input Size(array size(n))'; \
	    set ylabel 'Time(microseconds)'; \
	    set title 'heapsort vs quicksort vs mergesort with UNIFORM DISTRIBUTION RANDOM inputs'; \
...
gnuplot -e "set terminal pngcairo size 1000,800; \
	 ...
	 set xlabel 'Input Size(array size(n))'; \
	    set ylabel 'Time(microseconds)'; \
	    set title 'heapsort vs quicksort vs mergesort with WORST CASE Inputs'; \
